#!/usr/bin/env python3
"""Montagem do EP11 — A Tentação no Deserto (padrão EP03/EP09: multi-plano, cortes secos na cena,
...
CENAS = [
    ...
    ("c2", ["c2_0"], ["c2p1","c2p2","c2p3"], False, [("vento-suave",1.0,0.18)]),
    ("c3", ["c3_0"], ["c3p1","c3p2","c3p3"], False, [("whoosh-suave",4,0.20)]),
    ("c4", ["c4_0"], ["c4p1","c4p2","c4p3"], False, [("vento-suave",4,0.18)]),
    ("c5", ["c5_0"], ["c5p1","c5p2","c5p3"], False, [("whoosh-suave",6,0.20),("tcham-presente",16,0.25)]),
    ("c6", ["c6_0"], ["c6p1","c6p2"], False, [("whoosh-suave",2,0.20),("sininhos-v2",6,0.22)]),
    ("c7", ["c7_0"], ["c7p1","c7p2"], False, [("harpa-v2",1.0,0.18),("sininhos-v2",8,0.22)]),
    # REFLEXÃO: 3 lições + bordão + tchau — request único, trilha própria
    ("c10", ["c10_full"], ["c2p2","c3p3","c5p3","c6p2","c1p1"], False, [("sininhos-v2",0.5,0.22)]),
]
...
final = os.path.join(EP,"EP11-PREVIEW-A-TENTACAO-NO-DESERTO.mp4")
