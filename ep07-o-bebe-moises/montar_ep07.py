#!/usr/bin/env python3
"""Montagem do EP07 — O Bebê Moisés (padrão EP03: multi-plano, cortes secos na cena,
...
CENAS = [
    ...
    ("c3", ["c3_0"], ["c3p1","c3p2","c3p3"], False, [("aguinha",12,0.25)]),
    ("c4", ["c4_0"], ["c4p1","c4p2","c4p3"], False, [("passarinho-piu",6,0.18)]),
    ("c5", ["c5_0"], ["c5p1","c5p2","c5p3"], False, [("aguinha",1.5,0.22),("surpresa-fofa",10,0.28)]),
    ("c6", ["c6_0"], ["c6p1","c6p2","c6p3"], False, [("tcham-presente",14,0.25),("sininhos-v2",17,0.22)]),
    ("c7", ["c7_0"], ["c7p1","c7p2","c7p4","c7p3"], False, [("harpa-v2",1.0,0.18),("sininhos-v2",11,0.22)]),
    # REFLEXÃO: 3 lições + bordão + tchau — request único, trilha própria
    ("c10", ["c10_full"], ["c1p2","c3p3","c5p3","c6p2","c1p1"], False, [("sininhos-v2",0.5,0.22)]),
]
...
final = os.path.join(EP,"EP07-PREVIEW-O-BEBE-MOISES.mp4")
