...
- `name`: nome legível.
- `voice_id`: ID ElevenLabs (pt-BR) ou `-` para personagem sem fala.
...
Os visuais usam o MCP OpenArt autenticado por OAuth no Cursor. Os comandos
`gen-sheets.sh`, `gen-frames.sh` e `gen-clips.sh` criam um manifesto único com
todas as tarefas pendentes; peça ao agente para executar o manifesto em paralelo.
Depois que os resultados forem salvos, `register-openart.sh` normaliza a mídia,
valida os gates e vincula cada arquivo ao prompt e às referências exatas.
...
./gen-sheets.sh     # prepara build/openart/reference-batch.json
# agente: executar o lote com OpenArt MCP em paralelo
./register-openart.sh reference all
# revisar identidade/figurino e executar ./approve-visual.sh reference all
./gen-frames.sh     # prepara build/openart/frame-batch.json
# agente: executar o lote com OpenArt MCP em paralelo
./register-openart.sh frame all
# revisar composição/continuidade e executar ./approve-visual.sh frame all
./make-kit.sh
./gen-clips.sh       # prepara build/openart/clip-batch.json
# agente: executar o lote com OpenArt MCP em paralelo
./register-openart.sh clip all
...
O OpenArt MCP cobre imagens e vídeos. A dublagem, os SFX e a trilha usam
