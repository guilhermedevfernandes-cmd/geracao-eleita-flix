...
CENAS = [
    ...
    ("c3", ["c3_0"], ["c3p1","c3p2","c3p3"], False, [("harpa-v2",1.0,0.18)]),
    ("c4", ["c4_0"], ["c4p1","c4p2","c4p3"], False, [("whoosh-suave",6,0.20)]),
    ("c5", ["c5_0","c5_1"], ["c5p1","c5p2"], False, [("harpa-v2",1.0,0.18)]),
    ("c6", ["c6_0"], ["c6p1","c6p2","c6p3"], False, [("vento-triste",8,0.20)]),
    ("c7", ["c7_0","c7_1"], ["c7p1","c7p2"], False, [("harpa-v2",1.0,0.18)]),
    # c8 dividida em 2 blocos com pausa de 2s (só BGM) entre o castigo e o sinal de proteção
    ("c8", ["c8_a","c8_b"], ["c8p1","c8p2"], False, [("vento-suave",1.0,0.18),("sininhos-v2",10.8,0.22)]),
    # REFLEXÃO: 3 lições + bordão + tchau — request único, trilha própria
    ("c10", ["c10_full"], ["c1p2","c2p1","c4p1","c8p2","c1p1"], False, [("sininhos-v2",0.5,0.22)]),
]

# Durações customizadas por plano (segundos; None = divide o restante igualmente).
# c3: segura Caim (c3p1) até o fim da frase "Caim trouxe frutos da sua plantação" (~6.1s);
# c8: c8p1 cobre c8_a + pausa de 2s; c8p2 cobre o bloco do sinal de proteção.
PLANO_DUR = {
    "c3": [6.1, 4.4, None],
    "c8": [9.84, None],
}
...
for nome, segs, planos, pausa, sfxs in CENAS:
    gap_scene = 2.0 if nome == "c8" else (1.3 if nome == "c10" else GAP)
    ...
    custom = PLANO_DUR.get(nome)
    if custom:
        fixos = sum(d for d in custom if d is not None)
        livres = [i for i, d in enumerate(custom) if d is None]
        durs_pl = [d if d is not None else max(0.5, (total - fixos) / len(livres)) for d in custom]
    else:
        durs_pl = [total / len(planos)] * len(planos)
    vparts = []
...
final = os.path.join(EP,"EP04-PREVIEW-CAIM-E-ABEL.mp4")
