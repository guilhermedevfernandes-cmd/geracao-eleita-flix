#!/usr/bin/env python3
"""Montagem do EP13 — Daniel na Cova dos Leões (padrão EP12: multi-plano, cortes secos
na cena, fade branco entre cenas, BGM 0.10/0.065/0.08, reflexão com bgm própria,
clipes 0.75x, SEM sininhos). FREEZE nos planos de "estado final" (cova/anjo/proclamação)
para não reverter (leões "se agitando"/rei "abaixando o braço")."""
...
CENAS = [
    ...
    ("c2", ["c2_0"], ["c2p1","c2p2","c2p3"], False, []),
    ("c3", ["c3_0"], ["c3p1","c3p2"], True, []),
    ("c4", ["c4_0"], ["c4p1","c4p2","c4p3"], False, []),
    ("c5", ["c5_0"], ["c5p1","c5p2","c5p3"], False, []),
    ("c6", ["c6_0"], ["c6p1","c6p2","c6p3"], False, [("pedra-arrastando",18,0.25)]),
    ("c7", ["c7_0"], ["c7p1","c7p2","c7p3"], False, [("leao-rugido-fofo",1,0.30)]),
    ("c8", ["c8_0"], ["c8p1"], False, []),
    ("c9", ["c9_full"], ["c3p1","c2p2","c7p2","c7p3","c1p1"], False, []),
]

scene_files, scene_durs, sfx_events = [], [], []
t_cursor = INTRO
for nome, segs, planos, pausa, sfxs in CENAS:
    gap_scene = 1.3 if nome == "c9" else GAP
...
for sf, off, vol in sfx_events:
    ...
    idx += 1
fc = ";".join(filters) + f";{''.join(mixes)}amix=inputs={len(mixes)}:normalize=0,alimiter=limit=0.60:level=0[aout]"
final = os.path.join(EP,"EP13-PREVIEW-DANIEL-NA-COVA-DOS-LEOES.mp4")
