...
# sensível a áudio alto; narração sempre dominante. NUNCA sininhos/twinkle.
SFX_MAP = {
    "01": ("sfx/amb-nazare.mp3",  0.13),  # ambiência de vila (abertura em Nazaré)
    "09": ("sfx/viagem.mp3",      0.14),  # estrada p/ Belém (cascos, vento suave)
    "13": ("sfx/noite-campo.mp3", 0.14),  # noite no campo (grilos, ovelhas, fogueira)
    "16": ("sfx/coro-anjos.mp3",  0.16),  # multidão de anjos (coro etéreo, sem sinos)
    "24": ("sfx/estrela.mp3",     0.13),  # luz da estrela (pad aéreo quente, sem sinos)
}
...
def main() -> int:
    ...
    # alimiter limit=0.55 (garante <= -4 dB no MP4 mesmo com overshoot do AAC)
    # com level=0 (não re-normaliza).
    filt.append(
        "".join(mix_labels) +
