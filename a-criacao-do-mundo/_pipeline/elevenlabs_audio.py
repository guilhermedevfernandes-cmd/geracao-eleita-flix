...
TTS_MODEL = os.environ.get("ELEVENLABS_TTS_MODEL", TTS_JOB_TYPE)
TRANSIENT_MARKERS = (
    "http 429",
    "http 500",
    "http 502",
...
def convert_to_target(
    source: Path, output: Path, audio_filter: str | None = None
) -> None:
    if output.suffix.lower() == source.suffix.lower() and not audio_filter:
...
def generate_tts(
    text: str, voice_id: str, model: str, direction: str = ""
) -> bytes:
    directed_text = f"{direction.strip()} {text.strip()}".strip()
    return request_bytes(
        ...
        f"/v1/text-to-speech/{voice_id}",
        {
            ...
            "model_id": model,
            "voice_settings": {
                # O v3 usa Natural (0.5) para preservar a voz e ainda responder
                # às tags emocionais; Robust tende a achatar a interpretação.
                "stability": 0.5,
...
def create_audio(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="geflix-el-") as temporary:
            ...
            audio_filter = "atempo=0.95" if model == "eleven_v3" else None
            convert_to_target(raw, converted, audio_filter=audio_filter)
...
def command_tts(args: argparse.Namespace) -> int:
    ...
    return create_audio(
        ...
        values,
        lambda: generate_tts(
            args.text, args.voice_id, args.model, args.direction
        ),
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tts.add_argument("--model", default=TTS_MODEL)
    tts.add_argument(
        "--direction",
        default="",
        help="tags de direção vocal do Eleven v3, sem alterar a fala",
    )
