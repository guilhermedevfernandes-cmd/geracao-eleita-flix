...
while IFS=$'\t' read -r id act shot refs voice text hold image_prompt motion_prompt vfx sfx transition; do
  ...
  out="audio/${id}.mp3"
  direction=$(PYTHONPATH="$EPISODE_DIR/_pipeline" python3 -c \
    'import sys; from audio_contract import narration_direction; print(narration_direction(*sys.argv[1:4]))' \
    "$id" "$act" "$voice")
  echo "=== cena $id · ${VOICE_NAME[$voice]} · PT-BR ==="
  echo "    direção: $direction"
  args=(tts --model eleven_v3 --text "$text" --direction "$direction" --voice-id "$voice_id" --out "$out")
  (( FORCE )) && args+=(--force)
