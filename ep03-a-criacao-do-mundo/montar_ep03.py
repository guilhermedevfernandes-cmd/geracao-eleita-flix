#!/usr/bin/env python3
"""Montagem do EP03 — A Criação do Mundo (padrão EP02: multi-plano, cortes secos
dentro da cena, fade branco entre cenas, BGM 0.10/0.065/0.08, clipes a 0.75x)."""
...
GAP = 0.7        # entre segmentos de voz na mesma cena (ar entre Deus e narradora)
PAUSA = 2.2      # pausa interativa após (...)
RESPIRO = 2.5    # pausa entre um dia e outro, só com a BGM (pedido do usuário 12/07/2026)
INTRO = 2.0      # música antes da 1ª fala
OUTRO = 3.0      # cauda depois do tchau
SPEED = 0.75     # clipes em "colo de mãe"
FADE = 0.35      # fade branco entre cenas

# cena: (segmentos de áudio, planos, pausa_final?, sfx[(arquivo, offset_na_cena, vol)])
CENAS = [
    ("c1", ["c1_0"], ["c1p1","c1p2"], False, [("sininhos-v2",0.8,0.22)]),
    ("c2", ["c2_0"], ["c2p1","c2p2"], False, [("harpa-v2",1.0,0.18)]),
    ("c3", ["c3_0","c3_1"], ["c3p1","c3p2"], False, [("tcham-luz",1.4,0.30)]),
    ("c4", ["c4_0","c4_1"], ["c4p1","c4p2"], False, [("whoosh-fofo",1.4,0.25)]),
    ("c5", ["c5_0","c5_1"], ["c5p1","c5p2","c5p3"], False, [("splash-fofo",1.4,0.30),("bloom-magico",8,0.25)]),
    ("c6", ["c6_0","c6_1"], ["c6p1","c6p2","c6p3"], False, [("twinkle-estrelas",6,0.22)]),
    ("c7", ["c7_0","c7_1"], ["c7p1","c7p2","c7p3"], True, [("bolhinhas",5,0.28),("passarinho-piu",10,0.18)]),
    ("c8", ["c8_0","c8_1","c8_2"], ["c8p1","c8p2","c8p3"], False, [("sininhos-v2",12,0.22),("harpa-v2",15,0.18)]),
    ("c9", ["c9_0"], ["c9p1"], False, [("harpa-v2",1.0,0.18)]),
    # REFLEXÃO (teste EP03): 3 lições p/ mentalizar + tchau · trilha própria bgm-reflexao
    # c10 gravada em REQUEST ÚNICO (sotaque consistente — v3 não aceita previous_text)
    ("c10", ["c10_full"], ["c1p2","c9p1","c5p2","c8p3","c9p2"], False, [("sininhos-v2",0.5,0.22)]),
]

scene_files, scene_durs, sfx_events = [], [], []
t_cursor = INTRO
for nome, segs, planos, pausa, sfxs in CENAS:
    # ---- áudio da cena: segmentos + gaps + pausa + respiro ----
    gap_scene = 1.3 if nome == "c10" else GAP  # reflexão: ar entre as lições
    ...
    # ---- vídeo da cena: planos igualmente divididos, 0.75x, ping-pong se faltar ----
    ...
    for j, pl in enumerate(planos):
        ...
        vparts.append(part)
    # concat planos (cortes secos) + fade branco nas bordas da cena
...
# ---- vídeo final: intro branco + cenas + outro branco ----
...
# ---- áudio final: narrações posicionadas + BGM (0.10→0.065→0.08) + SFX ----
inputs = ["-i", vep]
filters, mixes = [], []
idx = 1
t = INTRO
for _, a in scene_files:
    ...
    t += dur(a); idx += 1
fim = total_ep
t_reflex = INTRO + sum(scene_durs[:-1])   # início da cena 10 (reflexão)
...
final = os.path.join(EP,"EP03-PREVIEW-A-CRIACAO-DO-MUNDO.mp4")
