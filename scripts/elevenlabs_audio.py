...
def generate_tts(
    ...
    # Workflow v3 (12/07/2026): modelo eleven_v3 + stability 1.0 ("Robusto",
    # como no ElevenLabs Studio) para consistência entre clipes. A expressão
    # vem das AUDIO TAGS no próprio texto — [whispering], [thoughtful] etc.
    # v3 só aceita stability 0.0 (Creative) / 0.5 (Natural) / 1.0 (Robust).
    # previous_text/next_text: contexto de prosódia entre segmentos consecutivos
    # (evita variação de sotaque/registro em frases curtas — o "virou carioca").
    payload = {
        "text": text,
        "model_id": TTS_MODEL,
    }
    if TTS_MODEL.startswith("eleven_v3"):
        payload["voice_settings"] = {"stability": 1.0}
    if previous_text:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tts.add_argument("--previous-text", default=None)
    tts.add_argument("--next-text", default=None)
    tts.add_argument("--seed", default=None, type=int)
