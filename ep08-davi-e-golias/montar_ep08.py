#!/usr/bin/env python3
"""Montagem do EP08 — Davi e Golias (padrão EP03: multi-plano, cortes secos na cena,
...
CENAS = [
    ...
    ("c3", ["c3_0"], ["c3p1","c3p2"], False, [("harpa-v2",1.0,0.18)]),
    ("c4", ["c4_0"], ["c3p3","c4p1","c4p2","c4p3"], False, [("vento-suave",6,0.18)]),
    ("c5", ["c5_0"], ["c5p1","c5p2","c5p3"], False, [("whoosh-suave",14,0.20)]),
    ("c6", ["c6_0"], ["c6p1","c6p2"], False, [("aguinha",1.0,0.22)]),
    ("c7", ["c7_0"], ["c7p1","c7p2","c7p5","c7p6","c7p3"], False, [("whoosh-suave",12,0.22),("tcham-presente",20,0.25)]),
    # REFLEXÃO: 3 lições + bordão + tchau — request único, trilha própria
    ("c10", ["c10_full"], ["c2p1","c3p2","c6p1","c7p3","c1p1"], False, [("sininhos-v2",0.5,0.22)]),
]
...
final = os.path.join(EP,"EP08-PREVIEW-DAVI-E-GOLIAS.mp4")
