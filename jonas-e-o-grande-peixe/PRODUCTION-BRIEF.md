...
6. `./gen-sheets.sh` + `./approve-visual.sh reference all`: identidade e figurino.
7. `./gen-frames.sh` + `./approve-visual.sh frame all`: composição e continuidade.
8. `./gen-clips.sh` ou produção manual + `./register-clips.sh`: animação vinculada ao frame e ao prompt.
